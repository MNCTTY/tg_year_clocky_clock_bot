...
def communication(message):
    ...
    ### у каждого пользователя он будет свой
    ### можно вычислить по now дате и тому, между чем и чем она находится
    
    
    ### а еесли бот падает нахуй, то пользователю его надо перезапускать 
    ### или он сам как-то подтягивается и продолжает молча работать?
    
    ### а что с нагрузкой на сервера??? счетчики-то для КАЖДОГО пользователя запускаются, 
